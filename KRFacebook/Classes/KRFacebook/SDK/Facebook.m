...
- (void)authorize:(NSArray *)permissions {
    ...
    self.session = [[[FBSession alloc] initWithAppID:_appId
                                         permissions:permissions
                                     urlSchemeSuffix:_urlSchemeSuffix
                                  tokenCacheStrategy:self.tokenCaching] autorelease];

    //強制使用 WebView 在當前登入
    [self.session openWithBehavior:FBSessionLoginBehaviorForcingWebView
                 completionHandler:^(FBSession *session, FBSessionState status, NSError *error) {
        
        //NSLog(@"session.accessToken x : %@", session.accessToken);
        switch (status) {
            case FBSessionStateOpen:
                //NSLog(@"here FBSessionStateOpen ?");
...
- (void)setupHasUpdatedAccessToken:(BOOL)_theHasUpdatedAccessToken{
    self.hasUpdatedAccessToken = _theHasUpdatedAccessToken;
}
